...
# Fit the pipeline
rf.fit(sold[features], y_sold)

# Predict prices for the listing data
predicted_prices = rf.predict(listing[features])

# Calculate the mean absolute percentage error (MAPE) of the model on the sold data
y_pred_sold = rf.predict(sold[features])
mape = calculate_mape(y_sold, y_pred_sold)
print(f"MAPE on sold data: {mape:.2f}%")

# Calculate MAPE for listing data (excluding NaN values)
valid_mask = ~listing['CURRPRICE'].isna() & (listing['CURRPRICE'] != 0)
y_true_listing = listing.loc[valid_mask, 'CURRPRICE']
y_pred_listing = predicted_prices[valid_mask]
mape_listing = calculate_mape(y_true_listing, y_pred_listing)
print(f"MAPE on listing data: {mape_listing:.2f}%")

# Create a price range based on the MAPE
listing['PREDICTED_PRICE'] = predicted_prices
listing['PRICE_MIN'] = listing['PREDICTED_PRICE'] * (1 - min(mape_listing, 100) / 100)
listing['PRICE_MAX'] = listing['PREDICTED_PRICE'] * (1 + min(mape_listing, 100) / 100)
...
for feature in ['ABGSQFT', 'INCOME', 'CARRY_COSTS', 'PROPTAX', 'YEARBUILT']:
    print(f"\n{feature}:")
    print(listing[feature].describe())
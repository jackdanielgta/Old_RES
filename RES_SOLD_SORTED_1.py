...
# Custom sorting function for TAX_BIN
def extract_numeric_range(tax_bin):
    if tax_bin == '>50000':
        return (50000, float('inf'))
    match = re.match(r"(\d+)-(\d+)", str(tax_bin))
    if match:
        return int(match.group(1)), int(match.group(2))
    return float('inf'), float('inf')  # Ensure that any invalid bins are sorted to the end

# Apply the extraction function and sort the DataFrame
sold['TAX_BIN_RANGE'] = sold['TAX_BIN'].apply(extract_numeric_range)
sold = sold.sort_values(by=['CITY', 'PROPTYPE', 'DIRECT', 'TAX_BIN_RANGE'])
sold = sold.drop(columns=['TAX_BIN_RANGE'])

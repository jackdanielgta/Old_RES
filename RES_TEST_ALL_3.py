...
rental = rental[["MLS1", "PROPTYPE", "ABGSQFT", 'TOT_SQFT', "CLOSEPRICE", "CITY", "MHIA21", "AHIA21", "BEDSTOTAL", "BATHSTOTAL", "DIRECT"]]
...
# Define custom bin edges for TOT_SQFT
bin_edges = [0, 1000, 2000, 3000, 4000, 5500, np.inf]  # Example ranges, adjust as needed
bin_labels = [0, 1, 2, 3, 4, 5]  # Corresponding labels for the bins

# Bin the TOT_SQFT into custom ranges
...
def calculate_carry_costs(df, price_col):
    # Replace missing values in HOA with 0
...
def calculate_stats_r(row, rental_df):
    # Bin the TOT_SQFT for the current listing row
    bin_label = pd.cut([row['TOT_SQFT']], bins=bin_edges, labels=bin_labels, right=False)[0]

    if row['DIRECT'] == 'N':
        if row['PROPTYPE'] == 'SF':
            # Filter rental DataFrame for Single Family properties
            filtered_rental = rental_df[
                ...
                (rental_df['PROPTYPE'] == 'SF') &
                (rental_df['DIRECT'] == 'N') &
                (rental_df['TOT_SQFT_BINNED'] == bin_label)
            ]
        else:
            # Filter rental DataFrame for all other property types
            filtered_rental = rental_df[
                (rental_df['CITY'] == row['CITY']) &
                (rental_df['BEDSTOTAL'] == row['BEDSTOTAL']) &
                (rental_df['INCOME'] >= row['LOWER_BOUND']) &
...
final_result[['MLS1', 'CITY', 'PROPTYPE', 'PROPTAX', 'LOWER_BOUND', 'UPPER_BOUND', 'MED_P_ACRE', 'PROP_ACRE', 'MED_P_SQFT', 'PROP_SQFT', 'MED_DOM', 'MIN_PRICE', 'MAX_PRICE', 'MIN_PSQFT', 'MAX_PSQFT', 'SIZE_COUNT', 'MAX/MIN_PSQFT', "OWNERSHIP_%", "POP_DENSITY", "MORT_PAY", "CARRY_COSTS", "COMPLETE_PAYMENT", "INCOME", "DTI_RATIO", "DISTANCE", 'RENT_PRICE', 'RENT_SQFT', 'RENT_COUNT', "INCOME", '%_RANK_PRICE', '%_RANK_AGE', '%_RANK_CARRY_COSTS', '%_RANK_P_SQFT', '%_RANK_P_ACRE', 'VACANCY', 'CURRPRICE', 'BATHSTOTAL', 'BEDSTOTAL', 'DOM', "CASHFLOW"]].to_csv(r'C:\cctaddr\RES-STATS.csv', index=False)
final_result.to_csv(r'C:\cctaddr\RES_FULL_DATA.csv', index=False)